...
const cardClasses = cva(["st-simple-card", "s-bg-gray-100 s-inline-flex s-p-3 s-rounded-lg"], {
  variants: {
    type: {
...
export const SimpleCard: FC<ICard> = forwardRef<HTMLDivElement, ICard>((props, ref) => {
  ...
  return (
    <Group className={cardClasses({ className, type })} ref={ref}>
      {icon && <Icon source={icon} className="s-mb-1 s-text-gray-400" />}
      <div className={type === "vertical" ? "" : "s-ml-2"}>
        <p className="s-mb-1 s-text-sm s-font-bold">{title}</p>
        <p className={`s-text-sm s-text-gray-500 ${type === "vertical" ? "s-mb-3" : ""}`}>{description}</p>
      </div>
      {type === "vertical" ? (
        <Button intent="tertiary" size="small" className="s-bg-white-900 s-font-medium" onPress={onPressButton}>
          {button}
        </Button>
      ) : (
