...
const cardClasses = cva(["st-hotel-card s-inline-flex s-flex-col", ""], {
  variants: {
    type: {
      compact: "",
      full: "s-border-[1px] s-border-gray-100 s-rounded-md s-overflow-hidden",
    },
  },
});
  ...
  return (
    <Group className={cardClasses({ className, type })} ref={ref} {...rest} onClick={onPress}>
      ...
      <div className={contentClasses({ type })}>
        ...
        >
          <p className="s-text-sm s-font-bold">{name}</p>
        ...
        <div className="s-flex s-items-center s-gap-1 -s-order-1">
          ...
          <p className="s-text-xs s-font-normal s-text-blue-500">{ratingLabel}</p>
          <p className="s-text-xs s-font-normal s-text-gray-300">{ratingHelper}</p>
        </div>
        <p className={`s-text-xs s-text-gray-500 ${type === "compact" ? "" : "-s-order-2"}`}>{location}</p>
        {type === "compact" && (
          <div className="s-flex s-gap-1">
            <p className="s-text-xs s-font-bold">{price}</p>
            {saleAtPrice && <p className="s-text-xs s-font-normal s-text-gray-300 s-line-through">{saleAtPrice}</p>}
          </div>
        )}
        {type === "full" && (
          <>
            <div className="s-flex s-flex-col s-gap-2 s-border-b-[1px] s-border-dashed s-border-gray-100 s-w-full">
              {labels}
              {voucher}
            </div>
            <div className="s-flex s-flex-col s-w-full s-items-end">
              ...
              {saleAtPrice && <p className="s-text-xs s-font-normal s-text-gray-300 s-line-through">{saleAtPrice}</p>}
              <p className="s-text-sm s-font-bold s-text-blue-500">{price}</p>
            </div>
          </>
        )}
      </div>
